 ...
 */
function generateSummary(releases: Release[]): Summary {
  const sortedReleases = [...releases].sort((a, b) => compareVersions(b.gbVersion, a.gbVersion));

  return {
    totalReleases: releases.length,
    totalPRs: releases.reduce((sum, r) => sum + r.totalPRs, 0),
    totalContributors: releases.reduce((sum, r) => sum + r.contributors, 0),
    avgPRsPerRelease: Math.round(
      releases.reduce((sum, r) => sum + r.totalPRs, 0) / releases.length
    ),
    latestRelease: sortedReleases[0]?.gbVersion ?? '',
...
async function main() {
  ...
  const summary = generateSummary(enrichedReleases);
